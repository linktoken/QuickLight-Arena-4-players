...
// Constants
const long debounceDelay = 100;          // Debounce time for buttons (in ms)
...
void handleGameEnd() {
  // Turn off all LEDs before resetting to avoid flashes
  for (int i = 0; i < 4; i++) {
...
void resetGame() {
  // Turn off all player LEDs before starting reset
  for (int i = 0; i < 4; i++) {
    digitalWrite(ledPins[i], LOW);  // Ensure all LEDs are off
  }

  delay(100);  // Optional delay to ensure all outputs are stable before reset

  // Re-enable player buttons and reset game state
  for (int i = 0; i < 4; i++) {
