// Define pins for buttons and LEDs
const int buttonPins[] = {2, 3, 4, 5};  // Pins for player buttons
const int ledPins[] = {6, 7, 8, 9};     // Pins for player LEDs
const int resetButtonPin1 = 10;         // Pin for the first reset button
const int resetButtonPin2 = 12;         // Pin for the second reset button
const int resetLEDPin = 11;             // Pin for reset LED
const int standbyLEDPin = 13;           // Pin for standby LED

// Define variables
int winner = -1;            // Player who has won (-1 means no winner)
bool gameEnded = false;     // Flag to indicate if the game has ended
unsigned long previousMillis = 0; // Store the last time the LED was updated
const long interval = 500;  // Interval for blinking (500 milliseconds)

// Debounce timing
const int debounceDelay = 50; // Debounce delay in milliseconds
unsigned long lastButtonPressTime[4] = {0, 0, 0, 0}; // Last button press time for each player

void setup() {
  // Initialize button pins as inputs with pull-up resistors
  for (int i = 0; i < 4; i++) {
    pinMode(buttonPins[i], INPUT_PULLUP);
  }
  
  // Initialize LED pins as outputs
  for (int i = 0; i < 4; i++) {
    pinMode(ledPins[i], OUTPUT);
  }
  
  // Initialize reset button pins as inputs with pull-up resistors
  pinMode(resetButtonPin1, INPUT_PULLUP);
  pinMode(resetButtonPin2, INPUT_PULLUP);
  
  // Initialize reset LED pin as output
  pinMode(resetLEDPin, OUTPUT);

  // Initialize standby LED pin as output
  pinMode(standbyLEDPin, OUTPUT);
  digitalWrite(standbyLEDPin, HIGH); // Turn on standby LED initially
}

void loop() {
  // If the game has not ended, continue to check buttons
  if (!gameEnded) {
    checkButtons();
  }
  
  // If a winner is declared, light up their LED and disable all other inputs
  if (gameEnded) {
    handleGameEnd();
  }
}

void checkButtons() {
  // Check each player's button
  for (int i = 0; i < 4; i++) {
    // Debounce logic
    if (digitalRead(buttonPins[i]) == LOW) {
      unsigned long currentTime = millis();
      if (currentTime - lastButtonPressTime[i] > debounceDelay) {
        lastButtonPressTime[i] = currentTime; // Update last press time
        // Player i has pressed the button
        winner = i;
        gameEnded = true;
        break;
      }
    }
  }
}

void handleGameEnd() {
  // Light up the winner's LED and turn off others
  for (int i = 0; i < 4; i++) {
    if (i == winner) {
      digitalWrite(ledPins[i], HIGH); // Light up winner's LED
    } else {
      digitalWrite(ledPins[i], LOW);  // Turn off other LEDs
      pinMode(buttonPins[i], INPUT);  // Disable other buttons
    }
  }
  
  digitalWrite(standbyLEDPin, LOW); // Turn off standby LED
  
  // Loop that both blinks the reset LED and checks the reset buttons
  while (digitalRead(resetButtonPin1) == HIGH && digitalRead(resetButtonPin2) == HIGH) {
    blinkResetLED();
  }

  // Once reset button is pressed, reset the game
  resetGame();
}

void blinkResetLED() {
  unsigned long currentMillis = millis();
  if (currentMillis - previousMillis >= interval) {
    previousMillis = currentMillis;
    // Toggle the reset LED
    digitalWrite(resetLEDPin, !digitalRead(resetLEDPin));
  }
}

void resetGame() {
  // Turn off all LEDs
  for (int i = 0; i < 4; i++) {
    digitalWrite(ledPins[i], LOW);
    pinMode(buttonPins[i], INPUT_PULLUP); // Re-enable buttons
  }
  
  // Reset game state
  winner = -1;
  gameEnded = false;
  digitalWrite(resetLEDPin, LOW); // Turn off reset LED
  digitalWrite(standbyLEDPin, HIGH); // Turn on standby LED
}
